...
let magnitudeBuffer = [];
const MAG_BUFFER_SIZE = 10;      // 50 değerlik sliding window
let emaStd = null;               // EMA durumu
const EMA_ALPHA = 0.2;           // 0.1–0.3 arası oynayabilirsin
...
openEarable.sensorManager.subscribeOnSensorDataReceived((sensorData) => {
    ...
    switch (sensorData.sensorId) {
     ...
     // --- Movement / No movement with 50-sample sliding window + EMA on STD ---
    ...
    if (movementStatus && movementBar) {
        ...
        if (magnitudeBuffer.length >= 10) {
            ...
            const variance = magnitudeBuffer
                .map(v => (v - mean) ** 2)
                .reduce((a, b) => a + b, 0) / magnitudeBuffer.length;
            ...
            if (percentage < 20) {
                ...
                movementBar.classList.add("bg-warning");
            } else {
